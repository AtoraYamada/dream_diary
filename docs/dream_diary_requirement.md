...
10.  **【目覚め】** 「夢の書斎（マイページ）」の鏡をくりくでログアウト。ログアウト時、「夢から覚める」演出を経て、トップページ（現実）へ戻る。詳細は `3.4.1. 目覚めの儀式（ログアウト演出）` を参照。
...
- **トップページ演出：** 扉クリック時のCSSアニメーション（上下黒幕の瞬き）および音声「重厚な扉の開閉音」再生。
- **UI構成と表示トリガー:**
    - **初期状態**: 森の扉の傍らに、小さな「紙片」が落ちている。
    - **展開時**: 紙片をクリックすると小さな紙片が消え、代わりに画面中央に「殴り書きの紙片」がオーバーレイで表示される。背景の森は少し暗転（減光）させ、入力に集中させる。
...
- **インタラクションの方針（シームレスな体験）:**
    ...
    - **鉛筆の演出**: 入力フォーカス（Focus）が当たっている間のみ、カーソルが「鉛筆」に変化し、背後で「カリカリ」という「鉛筆の筆記音」がループ再生される。
...
- **入力制限:**
  - ブラウザのLocalStorage容量制限および通信エラー時のデータ肥大化を防ぐため、トップページの「殴り書き」には最大2,000文字の制限を設ける。
- **感情の排除:**
  - この段階では「感情（平穏、恐怖など）」の選択UIは表示しない。あくまで「内容の記録」に特化させ、感情の分類は夢の中の「編む（作成）」フェーズでの儀式として残しておく。
- **音声管理**
  - 音声推奨の文字を画面中央上部に、右上に音声のミュートONOFFボタン
  - モダンブラウザの音声自動再生制限をクリアするため、「扉をクリック」する動作を `AudioContext` をレジューム（有効化）するトリガーとして利用する。
...
- **ビジュアルコンセプト:** 
    ...
    - 背景は、奥へと収束していく廊下の構図に、チェス盤のように少し歪んだ床や、壁に光るシンボルが浮かぶような、シュールで幾何学的な雰囲気を組み合わせ、「夢の中」の不思議な玄関ホールを表現する。
- **切り替えUI（カード差し替え）:**
    - 控えめに重なっている方のカードをクリックすると、「シュッ」という「カードの摩擦音」と共に、後ろのカードが手前に滑り込んでくるアニメーションで切り替える。
- **遷移演出:** 成功時、「境界を越える音」とともに共通の瞬き演出（`5.6.2`参照）を経て「静寂の書斎」を表示。
- **認証エラー演出（消えゆく文字）:**
    - 認証失敗時、バインダー上の入力文字が「砂のように崩れ落ちて消える」アニメーションを実行。
    - 儚いエラーメッセージを表示。
    - サウンド：砂が流れる「サラサラ」という「砂の崩落音（さらさら）音」を同期。
- **フォント・アセットのロード管理:**
...
- **レイアウトと視点構成（正面図）:**
  ...
  - **隅（鏡）:** 部屋の隅にある「鏡」。クリックでログアウト演出（目覚める）を開始。
- **オブジェクト割当（ナビゲーション）:**
    - **巻物：** 夢を編む（作成機能 / 巻物UI）
    - **本棚：** 記憶の目録（一覧機能 / 背表紙UI）
    - **窓：** 夢の氾濫（特殊ギミック）
    - **鏡：** 目覚める（ログアウト処理）
- **実装仕様:**
  - **座標管理:** 背景画像に対するパーセント指定でクリックエリアを定義し、レスポンシブに対応する。
...
- **演出フロー:**
    1.  **鏡の反応:** ホバーすると鏡の表面が光る。
    2.  **第1の瞬き（閉眼）:** クリックすると「境界を越える音」とともに「瞬き」演出で画面が暗転する。
    3.  **覚醒の兆候（開眼）:** 目を開くと、トップページの森の画像が **強いぼかし（`filter: blur()`）** がかかった状態で表示される。まだ夢と現の境界が曖昧な状態を表現する。音響は霞がかったようなSEを重ねる。
    4.  **第2の瞬き（再閉眼・開眼）:** 再度、「瞬き」演出が入る。
    5.  **完全な覚醒:** 完全にピントが合った通常のトップページ（森の扉）が表示され、操作可能になる。

### 3.5. 夢を編む（作成・編集）- 機能名: 新規作成（編集も同じ構造を再利用。ただし、作成は書斎が背景、編集は一覧が背景となる）
巻物スクロールに直接文字を書き込む没入型UI。
...
- **入力フィールド仕様:**
    ...
    - **感情彩色（必須）:**
        ...
        - 選択時のUI演出：「インクの滴り音」とともに巻物の背景色が対応する色に染まる
        - **デフォルト色:** インク未選択状態では、見開きページの地色を適用
    - **タグ登録（オプション）:**
        - 詳細仕様は `3.5.1. タグ登録機能` を参照
        - 「登場人物」「場所」の2カテゴリで複数登録可能
    - **入力中の音響:** 「羽ペンの筆記音」がループ再生される
- **定着の儀式（保存フロー）:** (実際の機能名: 保存処理)
    - **新規作成保存時の演出** 詳細仕様は`3.5.2. モーダル方式による「夢を編む（作成・定着）」のサイクル`を参照
    - **更新時の演出:** 修正後の保存（Update）では、実装負荷とUXのバランスを考慮し、以下の軽量化された演出を実行する。
        ...
        5. 「瞬き」が完了（開眼）すると、直接「一覧画面」が表示される。
            - モーダルで編集画面が開くので、つまり、モーダル削除
        6. 最後に、今編集した本の背表紙が「キラリ」と「キラリという光の音」とともに点滅し、更新が完了したことを示す。
...
- **入力支援:**
    ...
    - また、すでに作成済みのタグが複数登録できないようにするためでもある
- **タグバッジの操作:**
...
- **B. 定着（保存の儀式）** - **段階1（実装済）と段階2（アセット待機中）に分離**:
  ...
  **【段階1】栞の発光演出（実装済）**
  ...
  - **栞の発光色**: 一瞬発光する。
  - **発光後の挙動（段階1時点）**:
    - 発光完了と同時に、巻物の中間エリアが縮小（CSS Height 0）。
    - **巻物縮小アニメーション時間**: **0.6秒**で巻物が収納される。
    - **音響**: **[巻物の収束音]**（シュッという収納音）。
    - 巻物の縮小完了と同時に瞬き（暗転）が入る。
  - **机上の縮小版巻物の表示制御**:
    - **瞬き中（暗転中）**: 縮小版巻物UI を再表示（段階2の処理に備える）。
    - **瞬き明け**: 縮小版巻物UI は表示のまま。これにより、「机の上に巻物が戻された」感覚を演出。
  - **モーダル消滅と書斎への復帰**:
  ...
  **【段階2】本の実体化アニメーション（アセット待機中・実装準備完了）**
  ...
  - **完了**:
    - 本がモーダル背後の本棚（ぼかされた書斎の奥）へ「吸入・飛翔音」とともに縮小しながら飛んでいく。
    - モーダルが閉じ、書斎のぼかしが解除される。
    - **本棚の画像が現在の蔵書数に応じた段階（小/中/大）へ更新**される。
    - これにより、個別の座標管理を回避しつつ、「書斎の蔵書が増えた」という全体的な変化をユーザーに実感させる。

### 3.6. 記憶の目録（一覧）- 機能名: 一覧表示・検索
本棚に背表紙が並ぶ形式で、日付順（降順選択）で固定表示。

- **構造:**
    - 「一覧用背景（壁アップ）（PNG）」＋「本棚本体（PNG）」＋「背表紙（PNG）」＋「タイトル（HTML縦書き）」。
    - 個別にクリック可能な「動的な背表紙パーツ」を生成・配置する。
- **ページネーション（動的本棚・瞬きロード）:**
    - 記録数に応じて本棚が埋まる。画面の左右端をクリックすることで、「画面スライド音」とともに隣の本棚へ移動する。
...
- **モーダル方式による表示:**
    ...
    - **閉じる時の演出:**
      ...
      - **瞬き演出は不要**。理由：視点移動がなく、単なる「画面表示の切り替え」のため、背景状態の変化のみで充分。ユーザーのアクションとして閉じるため、アニメーション演出もなし。
      - **音響**: **[重厚な閉本音]**
- **UI構造（見開き）:**
    - 本を見開いた状態でモーダル内に表示。
    - ページ遷移は複雑な3Dアニメーションではなく、JSやCSSを用いて簡易的な3D挙動とする。
    - 「ページをめくる音」とともにページネーション
- **忘却の儀式（削除演出）:** (実際の機能名: 削除処理)
    - **演出フェーズ**:
        1. 砂時計が「砂時計の回転音」とともに逆回転を始める。
        2. テキストエリアの文字が「インクの滲む音」とともにインクのように滲み、上に向かって霧散する。
...
本棚の傍にある「索引箱」は、記憶を探すための多角的な機能を提供する。クリックで起動すると、「索引箱の開閉音」とともに古い図書館のカードボックスのような引き出しが開き、その内部がモーダルとして表示される。この際、索引箱の開いた引き出し自体が、モーダルウィンドウ内のフレームとなる。
...
- **ヘッダー領域:**
    ...
    - **検索窓エリア:** 2種類の検索窓が配置される。
        ...
        2.  **本文検索窓:** `「記憶を探索...」`というプレースホルダーを持つ。夢日記のタイトルと本文を対象としたキーワードを入力する。

- **カードリスト領域:**
    - ユーザーが作成したすべてのタグが「タグカード」として並んで表示される。
    - 大量のタグがある場合は垂直方向にスクロール可能。
    - 五十音順に並んでいる
    - ホバー時には「索引カードをめくる音」がなる
...
2.  **検索条件の指定:** ユーザーは以下の操作を順不同で行える。
    - **タグの選択:** カードリストから任意のタグカードをクリックする。
        - クリックされたカードは「ピン留め音」とともに「選択状態」となり、右上に「ピン留め」のようなアイコンが表示される。
...
4.  **結果表示:** 瞬き明け後、本棚に戻る。
    - 指定した**全てのタグ**を含み、**かつ**、タイトルまたは本文中に指定した**全てのキーワード**が存在する夢日記のみが本棚に表示される（フィルタリング演出）。
...
- 五十音インデックスと、表示名・読みがな両対応のタグ絞り込み機能を実現するため、各タグは以下のデータを持つ。
    ...
    - **五十音インデックス（yomi_index）** 五十音インデックス（例: `た`）。新規タグ作成時、バックエンド（Ruby on Rails）で自動生成して設定される。「あ」「か」「さ」…「英」「他」。
...
- **視覚演出（パプリカ風）:** 
    - 「ガラスの歪む音」とともに窓の外が歪み、日記の感情彩色（色）がついた文字が液体のように窓から流れ込む。
    - 画面全体がカラフルに点滅
    - 画面中央に **「窓ガラスに浮かぶ光の鏡文字」** として断片を表示する。
    - **注視演出:** ユーザーが鏡文字にマウスホバー（モバイルでは長押し）している間のみ、文字が反転して「正像」となる。
- **制約と解除条件:**
...
- **夢の断片のメタデータ:**
    ...
    - **覚醒度判定（明晰夢チェック）：** 保存時に「これは夢だと気づいていたか」を記録。
    - **感情彩色：** 夢の雰囲気を「平穏・混沌・恐怖・高揚」から選択。背表紙の色に反映させ、後から修正（変更）可能。
- **モバイル・レスポンシブ仕様:**
...
- **保存失敗（例外）時の演出:**
    ...
    - **フィードバック:** PS1風ノイズ、「不協和音（グリッチ音）」とともに「記憶が定着しません。霧が深すぎます。」等のメッセージを表示。
...
| **一覧用背景（壁アップ）** | 本棚が置かれている場所の「壁面」をアップにした画像。目録（一覧）と断片（詳細）の共通ベース背景として使用。 | 全画面を覆うため透過不要。 |
...
| | **鉛筆/チャコールのカーソル** | 現実世界での入力中のみ使用する、羽ペンより素朴な筆記具。 | カーソル形状を定義するため。CSS処理のため`.cur`ではなく`.png` |
| **本棚ユニット** | **本棚本体** | 空の棚。書斎画面では縮小配置、一覧画面では等倍〜拡大配置して使い回す。ズーム（scale）に耐えるため、高解像度（2k）で作成。 | 本棚の遠景画像として使うため。一覧で本の背表紙を並べるため。 |
| | **本棚（蔵書テクスチャ（小））** | 本棚の上段だけが埋まった画像。 | 本棚の遠景画像を夢の数に応じて変えるため。 |
| | **本棚（蔵書テクスチャ（中））** | 本棚の中段まで埋まった画像。 | 本棚の遠景画像を夢の数に応じて変えるため。 |
| | **本棚（蔵書テクスチャ（大））** | 本棚の下段まで埋まった画像。 | 本棚の遠景画像を夢の数に応じて変えるため。 |
| | **索引箱の外装** | 本棚の傍に配置。検索表示へのクリッカブル要素。 | |
| | **索引箱の内装** | 索引箱のアップ時のフレーム。| |
...
| **巻物** | **巻物：上端/下端** | 巻き取り部分。入力欄の上下に **固定** で配置。中間部はHTML。スクロール時も位置不変。 | 不規則な形状を背景と合成するため。 |
| | **縮小版巻物：白紙** | 縮小版巻物。LocalStorage メモ無の場合の画像。 | LocalStorage メモ有無の視認するため。library.htmlのクリッカブル要素（新規作成へ移行）として使うため。 |
| | **縮小版巻物：書きかけ** | 巻き取り部分。LocalStorage メモ有の場合の画像。 | LocalStorage メモ有無の視認するため。library.htmlのクリッカブル要素（新規作成へ移行）として使うため。 |
...
| **UI/その他** | **羽ペンのカーソル** | 夢を編む時に使用する筆記具。 | カーソル形状を定義するため。CSS処理のため`.cur`ではなく`.png` |
...
| **夢を編む（作成・編集）** | インクの滴り音 | [彩色] 感情選択時。「ピチャン」という透明感のある音。 |
...
- **書斎オブジェクト（静止）**:
    ...
    - **白紙状態（メモなし）**: `縮小版巻物：白紙`画像を表示。
    - **書きかけ状態（メモあり）**: `縮小版巻物：書きかけ`画像を表示。
- **作成画面UI（動的）**: 上端パーツ（画像）＋ 中間エリア（HTML/CSS可変長）＋ 下端パーツ（画像）の3段構成。

#### 5.4.2. モーダル方式による画面切り替えアニメーションの整合性:**

- **書斎→夢を編む（新規作成）:**
    - **Step 1（書斎）:** 机の上の「縮小版巻物オブジェクト」をクリック。
    - **Step 2（移行）:** **「瞬き（暗転）」演出の実行**。
    - **Step 3（モーダル表示）:** 暗転明け、書斎背景に `filter: blur(8px) brightness(0.6)` を適用し、画面中央に巻物モーダルが出現して展開する（音響：巻物を広げる音）。

- **書斎→記憶の目録（一覧）:**
    - **Step 1（書斎）:** 奥の壁の「本棚オブジェクト」をクリック。ズームイン。（音響：ズーム移動音（入/出））
    - **Step 2（移行）:** **「瞬き（暗転）」演出の実行**。
    - **Step 3（モーダル表示）:** 暗転明け、本棚に背表紙が並んでいる。
...
- **詳細→編集（モーダル切り替え）:**
    ...
    - **Step 4（編集モーダル表示）:** 暗転明け、編集巻物モーダルが出現し、縦に伸長して「夢を編む（編集）」画面が表示される（音響：巻物を広げる音）。
...
1. **閉眼:** CSSの `transform: scaleY(1)` で黒い幕が上下から中央へ閉じる。
2. **静止/ロード (不定):** 通信が発生する場合、この状態でレスポンスを待機。
3. **開眼:** データの準備が整い次第、`scaleY(0)` へ戻り、黒い幕が中央から上下へ広がり、視界が開ける。
    - **音響:** 「瞬きの音」を利用

#### 5.6.3. 巻物（作成画面）のモーダル実装詳細:**
- **モーダル構造:**
    - モーダルオーバーレイが画面全体を覆い、背景に `filter: blur(8px) brightness(0.6)` を適用。
...
- **スクロール挙動**:
    - **対象**: ブラウザ全体ではなく、入力欄のみをスクロールさせる。上端・下端パーツは固定。
...
```css
:root {
    --color-peace: #a0a9a6;   /* 0: 平穏 */
    --color-chaos: #b07a70;   /* 1: 混沌 */
    --color-fear: #838387;    /* 2: 恐怖 */
    --color-elation: #ca9e63;   /* 3: 高揚 */
}
```
...
#### 5.6.9. 汎用音響利用想定
- 「選択・決定音」：UIボタン押下時。世界観に合う「コッ」という短い木製音。
