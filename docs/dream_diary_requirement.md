...
**使用箇所**: トップ→認証、認証→書斎、書斎→一覧、作成開始、保存完了、詳細→編集、等
...
- 4:3画面の外側には、「暗い石壁」を配置し、世界観を補強する（全画面共通の最背面）
...
| 中景 | 殴り書きの紙片 | 初期状態：扉の傍らに小さな「紙片」が落ちている。クリックで展開 |
...
| 前景 | 入館証カード | 「継続（ログイン）」「新規（サインアップ）」のカード |
| 前景（左上） | 戻るボタン | トップページへ戻る |
...
| 中景 | 本棚（蔵書テクスチャ） | 夢日記の冊数に応じて画像を切り替え（後述） |
| 中景 | 索引箱 | 本棚の傍に配置（`img_index_box_exterior.png`） |
| 前景（手前） | 縮小版巻物 | 机の上に配置。LocalStorageの有無で画像切り替え |
...
| 前景 | 巻物 | 縦スクロール。上端（画像）＋入力エリア＋下端（画像） |
...
| 明晰夢チェック | - | boolean | - | false | 「これは夢だと気づいていたか」 |
...
- `scratch_memo` があれば構造化して全フィールドを初期化
  ...
  - `dream_date`: 現在日付（デフォルト）
  - `tags`: 空配列
  - `lucid_dream`: false（デフォルト）
...
4. **縮小版巻物の再表示**（瞬き明け後、即時）
   - 机上の縮小版巻物が再表示される

5. **共通演出：本の実体化**（瞬き明け後）
   - 本が実体化アニメーション（半開き → 閉じる）
...
- **巻物の実装**: 上下固定フレーム（画像）+ 中間部の紙はHTML・CSSベース
- **紙の質感**: CSS（`repeating-linear-gradient`など）で表現
- **背景色**: 感情彩色（emotion_color）に応じたクラスで動的に変更
...
**演出フロー**:
1. **共通演出：画面スライド**（セクション3.2参照）

2. **共通演出：瞬き**（200ms、ラグ隠蔽）
...
**演出フロー**:
1. **ページ回転アニメーション**（400ms）
   - `transform: rotateY()` で3D回転
   - SE: `sfx_page_turn.wav`
