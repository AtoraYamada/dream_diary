...
| レート制限 | rack-attack（Phase外で実装） | ブルートフォース攻撃防止、5回/分（IP単位） |
...
**目的**: ブルートフォース攻撃（パスワード総当たり）の防止

**実装方針**:
- **Gem**: rack-attack 6.7+
- **対象エンドポイント**:
  - POST /api/v1/sessions（ログイン）
  - POST /api/v1/registrations（サインアップ）
- **制限値**: 5回/分（IPアドレス単位）
- **応答**: 429 Too Many Requests

**トレードオフ**:
- **メリット**:
  - セキュリティ向上（パスワード総当たり攻撃の防止）
  - サーバー負荷軽減（悪意ある大量リクエストのブロック）
- **デメリット**:
  - 正規ユーザーの誤ブロック可能性（同一IP配下の複数ユーザー）
  - 運用コスト増（監視・調整が必要）
  - VPN/Proxy利用者への影響
- **判断**: 基本機能完成後に実装（優先度: 中）
...
**実装時の考慮事項**:
- ホワイトリスト機能（管理者IPの除外）
- ユーザーへの適切なエラーメッセージ（「一時的にアクセス制限中」等）
- ログ記録とアラート設定
